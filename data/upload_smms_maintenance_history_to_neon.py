"""
Uploader script for SMMS Signal & Telecom Maintenance History data into Neon PostgreSQL Cloud.
Target: postgresql://neondb_owner:npg_PjDr9ftS1isK@ep-soft-flower-ax8umez8-pooler.c-4.us-east-2.aws.neon.tech/neondb?sslmode=require&channel_binding=require
"""
...
NEON_DB_URL = "postgresql://neondb_owner:npg_PjDr9ftS1isK@ep-soft-flower-ax8umez8-pooler.c-4.us-east-2.aws.neon.tech/neondb?sslmode=require&channel_binding=require"
...
def upload_to_neon(db_url=NEON_DB_URL):
    print("[*] Loading smms_maintenance_history.json...")
    ...
    print("[*] Creating table 'smms_maintenance_history', indexes and views...")
    schema_sql = """
    CREATE TABLE IF NOT EXISTS smms_maintenance_history (
        job_id VARCHAR(64) PRIMARY KEY,
        source_system VARCHAR(32) NOT NULL DEFAULT 'SMMS_SIGNAL_TELECOM',
        department VARCHAR(32) NOT NULL DEFAULT 'SIGNAL_TELECOM',
    ...
    CREATE INDEX IF NOT EXISTS idx_smms_mh_div ON smms_maintenance_history (division);
    CREATE INDEX IF NOT EXISTS idx_smms_mh_zone ON smms_maintenance_history (zone_code);
    CREATE INDEX IF NOT EXISTS idx_smms_mh_section ON smms_maintenance_history (section);
    CREATE INDEX IF NOT EXISTS idx_smms_mh_bsec ON smms_maintenance_history (block_section);
    CREATE INDEX IF NOT EXISTS idx_smms_mh_work ON smms_maintenance_history (work_type);
    CREATE INDEX IF NOT EXISTS idx_smms_mh_asset ON smms_maintenance_history (asset_type);
    CREATE INDEX IF NOT EXISTS idx_smms_mh_start ON smms_maintenance_history (actual_start);
    CREATE INDEX IF NOT EXISTS idx_smms_mh_crit ON smms_maintenance_history (criticality);
    CREATE INDEX IF NOT EXISTS idx_smms_mh_payload ON smms_maintenance_history USING gin (payload);

    -- Standard maintenance_history view for direct querying
    CREATE OR REPLACE VIEW maintenance_history AS SELECT * FROM smms_maintenance_history;

    -- Feed container table to store full document feed
    CREATE TABLE IF NOT EXISTS smms_maintenance_feed (
    ...
    for item in records:
        ...
        row = (
            item["job_id"],
            feed.get("source_system", "SMMS_SIGNAL_TELECOM"),
            feed.get("department", "SIGNAL_TELECOM"),
            sec_meta["zone"],
            sec_meta["zone_code"],
            div,
    ...
    print("[*] Storing full feed payload into smms_maintenance_feed...")
    cur.execute("""
    INSERT INTO smms_maintenance_feed (source_system, department, record_count, payload)
    VALUES (%s, %s, %s, %s);
    """, (
        feed.get("source_system", "SMMS_SIGNAL_TELECOM"),
        feed.get("department", "SIGNAL_TELECOM"),
    ...
    print("\n" + "="*70)
    print("VERIFICATION OF UPLOADED SMMS DATA ON NEON POSTGRESQL CLOUD")
    print("="*70)

    cur.execute("SELECT COUNT(*) FROM smms_maintenance_history;")
    total_count = cur.fetchone()[0]
    print(f"[*] Total rows in 'smms_maintenance_history': {total_count}")
    ...
    cur.close()
    conn.close()
    print("\n[OK] SMMS Maintenance History successfully persisted on Neon Cloud PostgreSQL!")
