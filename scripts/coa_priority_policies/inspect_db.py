...
print("=== CHECKING coa_priority_policies TABLE ===")

# Find table exact name
cursor.execute("""
    SELECT table_schema, table_name 
    FROM information_schema.tables 
    WHERE table_name ILIKE '%priority%' OR table_name ILIKE '%coa%';
""")
...
# Check details for coa_priority_policies
table_name = "coa_priority_policies"
cursor.execute(f"SELECT COUNT(*) FROM {table_name};")
cnt = cursor.fetchone()['count']
print(f"\nCurrent row count in {table_name}: {cnt}")
